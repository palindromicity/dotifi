...
MOCK_DATA = pathlib.Path(__file__).parent.parent.parent.joinpath(
    "resources/test_mock_data.pickle"
)
...
def test_mechanics(mocker):
    ...
    process_groups_api_get_process_groups = mocker.patch(
        "nipyapi.nifi.apis.process_groups_api.get_process_groups"
    )
    ...
    process_groups_api_get_process_groupsd = mocker.patch(
        "nipyapi.nifi.apis.ProcessGroupsApi.get_process_groups"
    )
    ...
    for key, value in mock_data.items():
        if key.startswith("ProcessGroupsApi.get_process_groups"):
            this_response = nipyapi.nifi.ProcessGroupsApi().get_process_groups(
                value["args"][0]
            )
            i = 0
        ...
        if key.startswith("nipyapi.canvas.get_flow"):
            ...
            assert (
                this_response.process_group_flow.id
                == value["return"].process_group_flow.id
            )
        if key.startswith("nipyapi.canvas.get_remote_process_group"):
            this_response = nipyapi.canvas.get_remote_process_group(value["args"][0])
            if this_response is not None:
                assert (
                    this_response.remote_process_group.id
                    == value["return"].remote_process_group.id
                )
        if key.startswith("nipyapi.canvas.list_all_input_ports"):
